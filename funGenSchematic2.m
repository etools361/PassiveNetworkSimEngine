...
% 数据结构：使用链式前向星无向图结构
%--------------------------------------------------------------------------
function [img] = funGenSchematic2(iType, Value, cellNode1, CellNode2, cellName, w0)
% netlist standard
% 生成无向图
...
for ii=1:m
    cNode = PathTemp(ii);
    [AxisOfSchNode, x, y] = funDrawNodeAndLine(h, x, y, PointLength, AxisOfSchNode, r, m, ii, cNode);
    if ii ~= m
        iTypeD = e(DeviceEdgeTemp(ii)).iType;
        ValueD = e(DeviceEdgeTemp(ii)).Value;
        funDrawDevice(iTypeD, ValueD, x, y, r);
        x = x + DeviceLength;
    end
end
xLoad = x;
...
for ii=1:m
    ...
    for jj=1:mB % 将隐藏的节点坐标记录下来
        ...
        if jj~=1
            y = y - DeviceLength;
            [AxisOfSchNode, x, y] = funDrawNodeAndLine(h, x, y, PointLength, AxisOfSchNode, r, m, 0, cNode);
            CrossInfo(cNode) = 2;
        end
        funDrawDevice(iTypeD, ValueD, y-DeviceLength, x, r);
    end
    funGenGND(x, y-DeviceLength, r);
...
for ii=1:idx
    ...
    if nDevice.isMarked == 0 && mod(ii, 2) == 1
        ...
        if P1(2) ~= P2(2)
            ...
            if x1 == x2 % 属于垂直桥
                ...
                if CrossInfo(node2) == 2 % 对地枝节上的才需要横向扩展
                    x = x1+0.5;
                    funGenLine([x1, x], [y0, y0], r); % 画水平线
                else
                    x = x1;
                end
                if node1 ~= 1 % 不是到GND的，需要画横线
                    funGenLine([x1, x], [y1, y1], r); % 画水平线
                end
            else % 属于水平桥
                if CrossInfo(node1) == 3 || CrossInfo(node2) == 3
                    funGenLine([y1, y1-1], [x2, x2], ~r); % 画垂直线
                    y1 = y1 - 1;
                end
                funGenLine([y0, y1], [x1, x1], ~r); % 画垂直线
            end
        else
        ...
        if Pv
            ...
            iTypeD = cDevice.iType;
            ValueD = cDevice.Value;
            funDrawDevice(iTypeD, ValueD, y1, x, ~r);
            if node1 == 1 % 到GND的
                funGenGND(x, y1, ~r);
            else
            end
    ...
    %         funGenLine([x, x], [y0, y1], r); % 画垂直线
            ...
            funGenLine([y, y], [x1, x1+dExpLength/2], ~r); % 画水平直线
            funGenLine([y, y], [x2-dExpLength/2, x2], ~r); % 画水平直线
            funDrawDevice(iTypeD, ValueD, x1+dExpLength/2, y, r);
            fprintf('DeviceId = %d\n', ii);
            if node1 ~= 1
...
RL  = Value(b);
if RL == 0
    text(x0+0.0, y0+0.15, 'I_o', 'FontSize',12, 'FontWeight', 'bold');
else
    text(x0+0.0, y0+0.15, 'V_o', 'FontSize',12, 'FontWeight', 'bold');
end
img = [];
axis equal;
% end
...
switch iType
    case 0 % V
        ...
        funGenText(x2+0.8, y2-0.55, r, Value, 'V');
        text(x2+1, y2+0.18, 'V_i', 'FontSize',12, 'FontWeight', 'bold');
        y0 = y1;
    case 1 % I
...
%     text(x-0.1, y+0.15, [Data2Suffix(Value, '0.2'), strUnit], 'FontSize',12, 'FontWeight', 'bold');
    text(x-0.7, y+0.35, [Data2Suffix(Value, '0.2'), strUnit], 'FontSize',12, 'FontWeight', 'bold');
else
%     text(x+0.1, y-0.9, [Data2Suffix(Value, '0.2'), strUnit], 'FontSize',12, 'FontWeight', 'bold');
    text(x+0.28, y-0.45, [Data2Suffix(Value, '0.2'), strUnit], 'FontSize',12, 'FontWeight', 'bold');
end
hold off;
