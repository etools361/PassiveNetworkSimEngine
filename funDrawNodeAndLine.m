...
% 绘制节点和线段
%--------------------------------------------------------------------------
function [AxisOfSchNode, x, y] = funDrawNodeAndLine(h, x, y, PointLength, AxisOfSchNode, r, m, ii, cNode)
nodeT = h{cNode};
...
for jj=1:mPoint
    if jj~=1
        if r==0
            x = x + PointLength;
            funGenLine([x-PointLength, x], [y, y], r);
        else
            y = y - PointLength;
            funGenLine([y-PointLength, y], [x, x], r);
        end
